...
<template>
  <div>
    <Row>
      <Col span="24">
      <Card>
        <p slot="title" style="height: 32px;" class="header-title">
          <Button type="primary" @click="alertAdd" icon="md-add">{{ $t('add_button') }}</Button>
          <Button type="primary" @click="$router.go(-1)" icon="md-arrow-back">返回列表</Button>
        </p>
        <Alert show-icon>
...
<script>
import { getRequest, add, edit, del } from "@/api/saas-fields";
