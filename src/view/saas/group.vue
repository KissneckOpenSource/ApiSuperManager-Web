    ...
    <Row>
      ...
      <Card>
        <div class="margin-bottom-15">
          <Button type="primary" v-has="'InterfaceGroup/add'" @click="alertAdd"
    ...
    <Modal v-model="modalSetting.show" width="668" :styles="{top: '30px'}" @on-visible-change="doCancel">
      ...
      <Form ref="myForm" :rules="ruleValidate" :model="formItem" :label-width="80">
        ...
        <!-- <FormItem label="组头像" prop="image">
          ...
          <Upload type="drag" :action="uploadUrl" :headers="uploadHeader" v-if="!formItem.image"
            :format="['jpg','jpeg','png']" :max-size="5120" :on-success="handleImgSuccess"
            :on-format-error="handleImgFormatError" :on-exceeded-size="handleImgMaxSize"
        ...
        <!-- <FormItem label="应用分组" prop="app_hash">
          <Select v-model="formItem.app_hash">
            <Option v-for="(v, i) in appGroup" :value="v.hash" :kk="i" :key="v.hash"> {{v.name}}</Option>
          </Select>
        </FormItem> -->
      </Form>
...
<script>
import { getList, changeStatus, add, edit, del } from "@/api/saas-group";
        ...
        on: {
          click: () => {
            getAll().then((response) => {
              ...
              vm.formItem.app_hash = currentRow.app_hash;
    ...
    return h(
      "Poptip",
      {
        props: {
          confirm: true,
          title:
            "您确定要删除此接口分组么? 如当前分组下仍有接口，将自动划归于默认分组！",
          transfer: true,
        },
        on: {
          "on-ok": () => {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        app_hash: [
          { required: true, message: "应用分组不能为空", trigger: "blur" },
        ],
      },
      buttonShow: {
        edit: true,
