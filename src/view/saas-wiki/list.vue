...
<template>
  <Layout style="height: 100%">
    <Menu mode="horizontal" theme="dark" active-name="list">
      ...
      <template v-else>
        <div class="wiki-logo" @click="$router.push({name: 'home'})">首页</div>
      </template>
      <div class="wiki-nav">
        <MenuItem name="list" to="/saas-wiki/list">
      ...
      <div class="wiki-layout-con">
        ...
        <Collapse>
          <Panel v-bind:key="index" v-for="(item, index) in groupInfo" :name="index.toString()">
            {{item.name}}【{{item.create_time}}】 <span style="float: right;"
              class="margin-right-20">接口数量{{item.api_info ? item.api_info.length : 0}} | 项目热度{{item.hot}}</span>
          ...
          <Form :label-width="80">
            ...
            <FormItem label="填写地址">
              <div class="interface-test-url">
                ...
                <Alert class="url">
                  <!-- <Input v-model="requestURLPrefix" placeholder="" style="width: calc(50% - 100px);"></Input> -->
                  {{requestURL}}
                  <!-- <Input v-model="requestURLSuffix" placeholder="" style="width: calc(50% - 100px);"></Input> -->
                </Alert>
              </div>
            </FormItem>
            <FormItem label="请求地址">
...
import "./list.less";
import { apiGroup, detail, logout, updateCode } from "@/api/saas-wiki";
