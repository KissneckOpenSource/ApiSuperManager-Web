...
<template>
  <Layout style="height: 100%">
      ...
      <div class="wiki-layout-con">
        ...
        <Collapse>
          ...
          >
            ...
            <span style="float: right" class="margin-right-20"
              >接口数量{{ item.api_info ? item.api_info.length : 0 }} |
              项目热度{{ item.hot }}</span
            >
            <p slot="content" v-if="item.api_info">
              <span
                v-bind:key="api_index"
                v-for="(api_item, api_index) in item.api_info"
                @click="showApiDetail(api_item.hash)"
    ...
    <Drawer
      :title="api_detail.info"
      v-model="show_detail"
      width="820"
      :mask-closable="true"
      @on-close="closeDrawer"
      :draggable="true"
      @on-resize-width="handleResize"
    >
          ...
          <Form :label-width="80">
            ...
            <FormItem label="填写地址">
              <div class="interface-test-url">
                ...
                <Alert class="url"
                  ><Input
                    v-model="requestURLPrefix"
                    placeholder=""
                    style="width: calc(50% - 100px)"
                  ></Input>
                  {{ requestURL
                  }}<Input
                    v-model="requestURLSuffix"
                    placeholder=""
                    style="width: calc(50% - 100px)"
                  ></Input
                ></Alert>
            ...
            <FormItem label="请求头部">
              <Form :label-width="120" :label-position="left">
                <FormItem
                  v-for="(header, ri) in header_data_test"
                  :key="ri"
                  :label="header.field_name"
                  style="margin-bottom: 10px"
                >
                  <Input
                    v-model="header.result_val"
                    placeholder=""
            ...
            <FormItem label="请求参数">
              <Form :label-width="120">
                <FormItem
                  v-for="(header, ri) in request_columns_test"
                  :key="ri"
                  :label="header.field_name"
                  style="margin-bottom: 10px"
                  :required="header.is_must === 1"
                >
                  <Input
                    v-model="header.result_val"
                    placeholder=""
