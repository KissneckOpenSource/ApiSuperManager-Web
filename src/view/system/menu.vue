  ...
  <div class="search">
    <Card>
      <Row class="operation">
        ...
        <Button :disabled="selectList.length === 0" @click="delAll" class="margin-right-5" icon="md-trash">批量删除</Button>
      ...
      <Row>
        ...
        <div class="tree-bar" :style="{maxHeight: maxHeight}">
          <Tree ref="tree" :data="data" show-checkbox @on-check-change="changeSelect" @on-select-change="selectTree">
          </Tree>
        ...
        <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
          ...
          <FormItem label="菜单类型" prop="level">
            <RadioGroup v-model="form.level">
              <Radio label="1" border>顶级菜单</Radio>
              <Radio label="2" border>二级菜单</Radio>
              <Radio label="3" border>页内按钮</Radio>
            </RadioGroup>
          </FormItem>
          ...
          <Form-item>
            <Button @click="submitEdit" :loading="submitLoading" type="primary" icon="ios-create-outline"
              class="margin-right-5">修改并保存</Button>
  ...
  methods: {
    ...
    delAll() {
      this.$Modal.confirm({
        ...
        onOk: () => {
          let vm = this;
          let ids = this.selectList.map((v) => v.id).toString();
